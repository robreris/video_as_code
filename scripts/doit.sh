...
ffmpeg -i "$intro_video" -i "$bumper_audio" -fflags +genpts -c:v copy -c:a aac -strict experimental \
...
for i in "${!images[@]}"; do
    ...
    # Create a video from the image and audio, ensuring the video matches the audio duration
    ffmpeg -loop 1 -framerate 1 -i "$image" -i "$audio" -t "$duration" -c:v libx264 -c:a aac -pix_fmt yuv420p \
        -shortest "$output"
...
ffmpeg -i "$outro_video" -i "$bumper_audio" -fflags +genpts -c:v copy -c:a aac -strict experimental \
